 ...
 * @version $Id$
 *
 * A global notion of synchronization for all chromattic session and all JCR session.	
 */
public class Synchronization {
	...
	/** Return a specified global context by its name.
	 * 
	 ...
	 * @return the global context related the life cycle
	 * @throws IllegalStateException if a context is already create for specified life cycle
	 * */
	public SynchronizedContext openContext(ChromatticLifeCycle lifeCycle) throws IllegalStateException {
		if(lifeCycle == null) throw new NullPointerException();
	...
	public void close(boolean save) {
		...
		//First save all global context(sessions)
		for(SynchronizedContext context : contexts.values()) {
			context.close(save);
		}
		
		//Now save all JCR session
