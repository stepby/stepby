...
public class ResourceEntityResolver implements EntityResolver {
	...
	public ResourceEntityResolver(ClassLoader loader, Map<String, String> systemIdToResourcePath) {
		...
		if(systemIdToResourcePath == null) throw new NullPointerException();
		
		this.systemIdToResourcePath = new HashMap<String, String>(systemIdToResourcePath);
		this.systemIdToSource = new java.util.concurrent.ConcurrentHashMap<String, byte[]>();
		this.loader = loader;
	}
	...
	public InputSource resolveEntity(String publicId, String systemId) throws SAXException, IOException {
		if(systemId != null) {
			...
			if(systemIdToSource.size() > 1000) systemIdToSource.clear();
			
			if(data != null && data.length > 0) return new InputSource(new ByteArrayInputStream(data));
		}
		return null;
	}
}
